...
def main(sim_file):
    ...
    with PdfPages(output_pdf_name) as output:
        ...
        packet_index = np.array(list(range(0,len(packets))))
                # check io group 0
        packets0_mask = packets['io_group']==0
        print('total number of packets', len(packet_index))
        print('size of io_group 0', len(packets['timestamp'][packets0_mask]))
        
        tstamp0=packets['timestamp'][packets0_mask]
        ptype0=packets['packet_type'][packets0_mask]
        ttype0=packets['trigger_type'][packets0_mask]
        print('first 200')
        print('timestamps',tstamp0[0:200])# ,packet_index[packets0_mask])
        print('packet type',ptype0[0:200]) #,packet_index[packets0_mask])
        print('trigger type',ttype0[0:200])#,packet_index[packets0_mask])
        print('next 200')
        print('timestamps',tstamp0[201:400])# ,packet_index[packets0_mask])
        print('packet type',ptype0[201:400]) #,packet_index[packets0_mask])
        print('trigger type',ttype0[201:400])#,packet_index[packets0_mask])
        print('next 200')
        print('timestamps',tstamp0[401:600])# ,packet_index[packets0_mask])
        print('packet type',ptype0[401:600]) #,packet_index[packets0_mask])
        print('trigger type',ttype0[401:600])#,packet_index[packets0_mask])
        print('next 200')
        print('timestamps',tstamp0[601:800])# ,packet_index[packets0_mask])
        print('packet type',ptype0[601:800]) #,packet_index[packets0_mask])
        print('trigger type',ttype0[601:800])#,packet_index[packets0_mask])
        ...
        ### Plot only timestamps from io_groupp 0 and pf pps resets
        print('size of io_group 0', len(packets['timestamp'][packets0_mask]))
        print('size of rollover mask', len(packets['timestamp'][rollover_packet_mask]))
        plt.plot(packets['timestamp'][packets0_mask],packet_index[packets0_mask],'o',label='packets io_g=0',linestyle='None')
        plt.xlabel('timestamp')
        plt.ylabel('packet index')
        plt.legend()
        output.savefig()
        ...
        plt.plot(packets['timestamp'][sync_packet_mask],packet_index[sync_packet_mask],'o',label='PPS(sync) packets',linestyle='None')
        plt.xlabel('timestamp')
        plt.ylabel('packet index')
        plt.legend()
        output.savefig()
