...
vertices_dtype = np.dtype([("eventID","u4"),("x_vert","f4"),("y_vert","f4"),
                           ("z_vert","f4"), ("nu", "i4"), ("tgt", "i4"), 
                           ("N", "i4"), ("proc", "S32"), ("res", "i4"), 
                           ("q", "S32")])

particle_stack_dtype = np.dtype([("eventID", "u4"), ("status", "u4"), ("pdgId", "i4"), ("p4", "f4", (4,) )])
...
def initHDF5File(output_file):
    with h5py.File(output_file, 'w') as f:
        ...
        f.create_dataset('particle_stack', (0,), dtype=particle_stack_dtype, maxshape=(None,))

# Resize HDF5 file and save output arrays
def updateHDF5File(output_file, trajectories, segments, vertices, particle_stack):
    if any([len(trajectories), len(segments), len(vertices), len(particle_stack)]):
        with h5py.File(output_file, 'a') as f:
            ...
            if len(particle_stack):
                nps = len(f['particle_stack'])
                f['particle_stack'].resize((nps+len(particle_stack),))
                f['particle_stack'][nps:] = particle_stack
...
def dump(input_file, output_file):
    ...
    gRooTracker = inputFile.Get("DetSimPassThru/gRooTracker")
    ...
    vertices_list = list()
    particle_stack_list = list()

    EvtNum = 0

    for jentry in range(entries):
        nb = inputTree.GetEntry(jentry)
        gRoo_nb = gRooTracker.GetEntry(jentry)

        event = inputTree.Event
        # write to file
        if len(trajectories_list) >= 10 or nb <= 0:
            updateHDF5File(
                output_file,
                ...
                np.concatenate(particle_stack_list, axis=0) if particle_stack_list else np.empty((0,)))
        ...
        for primaryVertex in event.Primaries:
            ...
            strcode = str(getattr(gRooTracker, "EvtCode") )
            codedict = dict()
            for subcode in strcode.split(";"):
                if subcode in ["", " "]: continue
                key, item = subcode.split(":")
                codedict[key] = item

            vertex["nu"] = int(codedict["nu"])
            vertex["tgt"] = int(codedict["tgt"])
            vertex["proc"] = codedict["proc"]
            if "N" in codedict.keys(): vertex["N"] = codedict["N"]
            else: vertex["N"] = 0
            if "q" in codedict.keys(): vertex["q"] = codedict["q"]
            else: vertex["q"] = ""
            if "res" in codedict.keys(): vertex["res"] = int(codedict["res"])
            else:  vertex["res"] = -1

            vertices_list.append(vertex)

        particle_stack = np.empty(len(getattr(gRooTracker, "StdHepStatus")), dtype=particle_stack_dtype)
        print(event.EventId, 'N particles:', len(getattr(gRooTracker, "StdHepStatus")))
        for ipart in range(len(getattr(gRooTracker, "StdHepStatus"))):
            particle_stack['eventID'][ipart] = event.EventId
            particle_stack['p4'][ipart] = (getattr(gRooTracker, "StdHepP4")[4*ipart], getattr(gRooTracker, "StdHepP4")[4*ipart+1], getattr(gRooTracker, "StdHepP4")[4*ipart+2], getattr(gRooTracker, "StdHepP4")[4*ipart+3])
            particle_stack['pdgId'][ipart] = getattr(gRooTracker, "StdHepPdg")[ipart]
            particle_stack['status'][ipart] = getattr(gRooTracker, "StdHepStatus")[ipart]
        particle_stack_list.append(particle_stack)
